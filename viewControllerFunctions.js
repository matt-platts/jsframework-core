...
/* 
 * Function: load
 * This is the first function called, and is a wrapper for makePageCallsInner.
...
page.prototype.makePageCallsInner = function(){
	...
	dataToSend = this.dataToSend;	
	    ...
	    pageCallInOp = true; // a page call is in operation.
	    if(!dataToSend) {
		...
		var length = arrayWStyleSheets.length,
		element = null;
		...
		var arrayWStyleSheets=pageRegister[pageIdentifier]["additionalStyleSheets"];
		arrayWStyleSheets=arrayWStyleSheets.split( "," );
	    ...
	    if(!calls) noAJAXRender(pageIdentifier);
            ...
            200: function(data, textStatus, XMLHttpRequest) {
                ...
                if(data["user"]["userType"]["PARENT"] == true) {
                          if (data["user"].joinFormStatus=="YOUR_CHILDREN"){
                              ...
                              //makePageCallsMobile("signupCardDetails", "signupCardDetailsM");
                              makePageCallsMobile("signupConfirmID", "signupConfirmIDM");
                          } else {
                             makePageCalls("home");
                          }
